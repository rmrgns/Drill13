...
class Zombie:
    ...
    def move_slightly_to(self, tx, ty, reverse=1):
    ...
    def compare_ball_count(self):
        if self.ball_count > play_mode.boy.ball_count:
            print(play_mode.boy.ball_count)
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('소년과 좀비의 공 개수 비교', self.compare_ball_count)
        root = SEQ_chase_boy = Sequence('공 개수 비교해서 추적 결정', c2, a4)

        a5 = Action('소년으로부터 도망', self.runaway_from_boy)


        root = SEL_chase_or_runaway = Selector('추적 or 도망', SEQ_chase_boy, a5)
        root = SEQ_find_boy = Sequence('소년을 탐지', c1, SEL_chase_or_runaway)
        root = SEL_find_or_flee = Selector('탐지 또는 배회', SEQ_find_boy, SEQ_wander)
